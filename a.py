...
def logo(n):
 ...
 selamat datang {hh}{n}{P}, script by {hh}andre x ridho{P} limited user""")
def logo2():
	return str(f"""  _   _ _______  _______    ____  _        _    ____  _____
 | \ | | ____\ \/ /_   _|  | __ )| |      / \  |  _ \| ____|
 |  \| |  _|  \  /  | |    |  _ \| |     / _ \ | | | |  _|
 | |\  | |___ /  \  | |    | |_) | |___ / ___ \| |_| | |___
 |_| \_|_____/_/\_\ |_|    |____/|_____/_/   \_\____/|_____|
 selamat datang {hh}user{P}, script by {hh}andre x ridho{P} limited user""")
...
def otomatis():
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			idf,nama = akun.split('|')[0],akun.split('|')[1].lower()
			depan = nama.split(" ")[0]
			pwx = ['sayang','sayangkamu','sayangku','katasandi']
			if len(nama)<=5:
				if len(depan)<=1 or len(depan)<=2:
					pass 
				else:
