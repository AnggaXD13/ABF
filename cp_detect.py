	...
	def cek_opsi(self):
		...
		if "c_user" in session.cookies.get_dict():
			if "Akun Anda Dikunci" in urlPost.text:
				if self.satua==True:
					print(f"\r\33[1;33m--> {self.user} | {self.pw}								\33[37;1m\n",end="")
				print(f"\r× Akun sesi new					\n\n",end="")
			else:
				aman+=1
				coki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
				if self.satua==True:
					print(f"\r{H}--> {self.user}|{self.pw}        ",end="")
				print(f"\r{H}✨ Hore, tinggal satu langkah lagi untuk membuka akun ini, coba login di browser{P}                   \n\n",end="")
			...
			if self.satua==True:
				print(f"\r\33[1;33m--> {self.user} | {self.pw}								\33[37;1m\n",end="")
			print(f"\r	• Terdapat {BM}{len(cek)}{P} opsi:\n",end="")
			if(len(cek)==0):
				...
				if "c_user" in session.cookies.get_dict():
					...
					coki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
					if self.satua==True:
						print(f"\r{H}[OK] {self.user}|{self.pw}        ",end="")
